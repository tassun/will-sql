...
export class PgSQLDBQuery {
    ...
    public static executeQuery(conn: PoolClient, query: string | KnSQLOptions, params?: KnDBParam | Array<any>) : Promise<KnResultSet> {
        let sql = KnDBUtils.getQuery(query);
        let [parameters] = Array.isArray(params) ? [params] : KnDBUtils.extractDBParam(params,"pg");
    ...
    public static executeUpdate(conn: PoolClient, query: string | KnSQLOptions, params?: KnDBParam | Array<any>) : Promise<KnResultSet> {
        let sql = KnDBUtils.getQuery(query);
        let [parameters] = Array.isArray(params) ? [params] : KnDBUtils.extractDBParam(params,"pg");
        return new Promise<KnResultSet>((resolve, reject) => {
            conn.query(sql,parameters,(qerr, rows) => {
